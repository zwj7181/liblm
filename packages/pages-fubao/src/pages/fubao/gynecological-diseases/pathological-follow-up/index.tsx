...
function showView(record: any, handleSearch: any) {
  mchcModal__.open('modal_form', {
    modal_data: {
      ...
      async onSubmit(data) {
        await request.put('/api/two/cancer/screening/updateTwoCancerPathologicalResultTracking', data);
...
export default function Pathological_follow_up(props: any) {
  ...
  return <MyBaseList genColumns={({ handleDelete, handleSearch }) => {
    ...
    get_fuck_page={async (config) => {
      ...
      return (await request.get('/api/two/cancer/screening/getTwoCancerPathologicalResultTracking', { ...config, unboxing: true })).data
    }}
    showRowDelBtn
    baseTitle='档案管理'
    showAdd={false}
    ...
    renderBtns={({ getSearchParams, getCheckRows, handleSearch }) => {
      ...
      return <>
        <OkButton primary disabled={!ids.length} onClick={() =>
          request.post('/api/two/cancer/screening/pushTwoCancerPathologicalResultBatch', { ids }, { successText: '操作成功' }).then(handleSearch)
        }>批量通知</OkButton>
      </>
    }}
    beforeSearch={(v) => {
    ...
    searchConfig={[
      ...
      { inputType: 'MS', name: 'notificationStatus', label: '随访状态', props: { options: queryFormDescriptions.notificationStatus.options } },
