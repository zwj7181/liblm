              ...
              <Collapse.Panel
                header={
                  !item.reportGestationalWeek
                    ...
                    : `${formatDate(item.reportDate)}  (孕周：${item.reportGestationalWeek})`
                }
                key={item.id}
              >
    ...
    return (
      <div className="survey-right">
        ...
        <ul className="right-msg">
          ...
          <li className="msg-item">报告日期: {formatDate(get(detailData, 'reportDate')) || ''}</li>
          <li className="msg-item">送检日期: {formatDate(get(detailData, 'requestDate')) || ''}</li>
        </ul>
        <div>
          {reportFile ? (
