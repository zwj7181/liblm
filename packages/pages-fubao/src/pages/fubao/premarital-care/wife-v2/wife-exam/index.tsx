...
export default class panel extends PanelWithChild {
  ...
  renderHeader = () => {
    ...
    const f_vm = get(data, 'manPremaritalCheckArchivesDetailVM')
    const h = [
  ...
  handleClickButton = () => {
    ...
    if (get(data, 'manPremaritalCheckArchivesDetailVM')) {
      fubaoHistoryPush(`/premarital-care/husband-v2/husband-exam?id=${id}`, this.props as any);
    } else {
      Modal.error({
  ...
  renderContent = () => {
    ...
    const sex_data = get(data, 'womanPremaritalCheckArchivesDetailVM')
