...
export default class CustomEditor extends React.Component<IProps> {
  ...
  handleUpload = async ({ file, progress, libraryId, success, error }) => {
    try {
      const { accessKeyId, accessKeySecret, securityToken } = await request.post('/api/getSTS');
      ...
      <>
        ...
        {editorVisible && (
          <Row>
            ...
            <Col span={7} offset={1}>
              <div className="custom-mobile-block">
                <div className="custom-mobile-block-content">
                  <iframe
                    id="iframe"
                    ref={(node) => {
                      this.iframeRef = node;
