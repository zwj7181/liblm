...
class EditInTable extends Component {
  ...
  constructor(props: any) {
    super(props);
    console.log(props, 567);
    this.tableColumns = safe_json_parse(get(props, 'config.specialConfig')).tableColumns;
    this.tableTitle = safe_json_parse(get(props, 'config.specialConfig')).tableTitle;
    this.srbls = safe_json_parse(get(props, 'config.specialConfig')).srbls;
    const tableRows = safe_json_parse(get(props, 'config.specialConfig')).tableRows;
    console.log(safe_json_parse(get(props, 'config.specialConfig')), '8123');
    this.state = {
      selectedRowKeys: [],
      dataSource: map(tableRows, (item) => {
        return {
          xm: item,
          key: -Math.random(),
        };
  ...
  setFormData = (dataSource) => {
    const formData = {};
    const allColumns = this.getAllColumnsMapping(this.tableColumns);
  ...
  getDataSource = () => {
    ...
    const allTableColumns = this.getAllColumnsMapping(this.tableColumns);
    return map(dataSource, (rowData) => {
      ...
      map(allTableColumns, (col: any) => {
        ...
        if (inputType === 'dictionary_select_in_table') {
          ...
          const optionMapping = keyBy(getDictionariesEnumerations, 'value');
  ...
  render() {
    ...
    const mergedColumns = this.getColumns(this.tableColumns);
    const mergedDataSource = this.getDataSource();
    return (
      <>
        <h1 style={{ textAlign: 'center', fontSize: '24px', marginTop: '60px' }}>{this.tableTitle}</h1>
        <Form
          ref={(formRef) => {
            this.form = formRef;
          }}
