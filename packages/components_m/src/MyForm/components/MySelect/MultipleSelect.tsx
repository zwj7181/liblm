...
export default class MultipleSelect extends Component<MultipleSelectProps> {
  ...
  render() {
    const { options = [], value, multiple = false, extraEditors = [], disabled } = this.props;
    let { selectValue = multiple ? [] : '', editorsValue = multiple ? [] : '' } = value;
    if (selectValue === '' && multiple) {
      selectValue = [];
    }
    if (editorsValue === '' && multiple) {
      editorsValue = [];
    }
    return (
      <div>
        ...
        {/* <div style={{ marginTop: 8 }}>{this.renderExtra(selectValue, editorsValue, extraEditors, multiple)}</div> */}
      </div>
    );
  }
}
