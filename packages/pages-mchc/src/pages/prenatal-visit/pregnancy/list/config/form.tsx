...
import { defineFormConfig, IMchc_Pregnancy } from '@lm_fe/service';
const ctx = rt_ctx
  ...
  [
    {
      ...
      filterType: 'contains',
      inputProps: {
        onPatientSelect(value, form) {
          const pregnancy: IMchc_Pregnancy = value
          if (!pregnancy) return
          const isUn = pregnancy.recordstate === '0'
          const id = pregnancy.id
          const url = isUn ? '/prenatal-visit/pregnancy/check?id=' : '/prenatal-visit/pregnancy/nurse-end?id='
          ctx.safeTo(url + id, { id })
        },
        width: 140,
      },
