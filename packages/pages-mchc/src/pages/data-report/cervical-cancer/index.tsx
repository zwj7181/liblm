...
export default function BreastCancerDataReport(prop: any) {
    ...
    return <Wrap>
        <MyBaseList
            // apiPrefix="/fb/api"
            name="/api/two/cancer/screening/findAllCervicalCancerReportedDataLog"
            searchParams={{
                // 'visitType.equals': 1,
            }}
            needChecked
            ...
            renderBtns={(ctx) => {
                ...
                return <Button.Group>
                    <Button disabled={!selectRows.length} onClick={async () => {
                        request.post('/api/dataReport/reportCervicalCancerRecordFlow', { ids: selectRows.map(_ => _.id), });
                        ctx.handleSearch()
                    }}>
                        结案
                    </Button>
                    <Button disabled={!selectRows.length} onClick={async () => {
                        request.post('/api/dataReport/reportCervicalCancerRecord', { ids: selectRows.map(_ => _.id), });
                        ctx.handleSearch()
                    }}>
                        上报
                    </Button>
