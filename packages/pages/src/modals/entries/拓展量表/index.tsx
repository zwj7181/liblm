...
function Index(props: IGlobalModalProps<{ headerInfo?: IMchc_Doctor_OutpatientHeaderInfo, ext: ICommonOption }>) {
  ...
  const { config, Wrap } = BF_Wrap2({ default_conf: { title: `拓展量表-${ext.value}` } })
  const base_args = {
    relationId,
    // type: ext_value,
  }
  async function fetch_data() {
    let res = await request.get(`/api/assessment/${ext_value}`, { params: base_args })
    setData(res.data)
  ...
  async function handleOk() {
    ...
    if (values) {
      const res = await request.put(`/api/assessment/${ext_value}`, values, { successText: '操作成功' })
      setData(res.data)
    }
    ...
    return (
      <Space style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <OkButton hidden={!data_id} onClick={() => mchcModal__.open('print_modal', {
          modal_data: { requestData: { url: `/api/assessment/${ext_value}/print`, template: ext_value, relationId, id: data_id } }
        })}>
          打印
        </OkButton>
